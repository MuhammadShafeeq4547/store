...
const AdminDashboard = () => {
  ...
  const getAuthHeaders = () => ({
    'Authorization': `Bearer ${"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY4ODlmODk4NjU4ZTZmYTE5MmUwMDFiYSIsImlhdCI6MTc1NDA1MzE1MCwiZXhwIjoxNzU2NjQ1MTUwfQ.WdIicvnvM0-wiaHzDTg3cGBB_D6wThrA8kYneu-pGmM"}`,
  ...
  const fetchCustomerDetails = async (customerId) => {
    try {
      ...
      handleApiError(error, 'Load customer details');
    }
  };
  ...
  const updateOrderDetails = async (orderId, details) => {
    try {
      ...
      handleApiError(error, 'Update order details');
    }
  };
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{dashboardData?.totalCustomers?.toLocaleString() || 0}</p>
              <p className="text-xs text-green-600 mt-2 flex items-center">
                <TrendingUp className="w-3 h-3 mr-1" />
                +{dashboardData?.customersGrowth || 0}% from last month
              </p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{dashboardData?.newCustomers?.toLocaleString() || 0}</p>
              <p className="text-xs text-green-600 mt-2 flex items-center">
                <TrendingUp className="w-3 h-3 mr-1" />
                Active growth
              </p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">${dashboardData?.avgOrderValue?.toFixed(2) || 0}</p>
              <p className="text-xs text-green-600 mt-2 flex items-center">
                <TrendingUp className="w-3 h-3 mr-1" />
                Increasing trend
              </p>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.reduce((customers, order) => {
                ...
                if (existingCustomer) {
                  ...
                  customers.push({
                    _id: order.user._id || email,
                    name: order.shippingAddress?.fullName || order.user?.name || 'N/A',
                    email: email,
                    totalOrders: 1,
                ...
                <tr key={index} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <p className="text-sm text-gray-500">Joined: {new Date(customer.joinedDate).toLocaleDateString()}</p>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      customer.totalSpent > 1000 ? 'bg-purple-100 text-purple-800' :
                      customer.totalSpent > 500 ? 'bg-green-100 text-green-800' :
                      'bg-gray-100 text-gray-800'
                    }`}>
                      {customer.totalSpent > 1000 ? 'VIP' : customer.totalSpent > 500 ? 'Premium' : 'Regular'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-2">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-green-600">${selectedCustomer.totalSpent?.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div key={index} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          ...
                          <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(order.status)}`}>
                            {order.status}
                          </span>
                        </div>
                      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage your e-commerce store efficiently</p>
            </div>
